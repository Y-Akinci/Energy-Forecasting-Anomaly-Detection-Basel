"""
MODELING FRAMEWORK V9 - 24H FORECAST (KORRIGIERTE FEATURE NAMEN)
=================================================================
1 Tag (24h) Forecast mit Top 20 Features (korrekte Namen aus CSV)
...
class Config:
    # Pfade
    OUTPUT_DIR = r"C:\Users\haris\OneDrive\Anlagen\Desktop\ML nach Model\Energy-Forecasting-Anomaly-Detection-Basel\Modeling\Haris\V9"
    ...
    # Forecast Horizon
    FORECAST_HORIZON_STEPS = 96  # 96 steps = 24h
    FORECAST_HORIZON_NAME = "24h"
    
    TRAIN_RATIO = 0.8
    CV_SPLITS = 5
    
    # Top 20 Features (KORRIGIERTE NAMEN aus CSV)
    TOP_FEATURES = [
        'Lag_15min',
        'Lag_30min',
        'Lag_1h',
...
print("=" * 70)
print("MODELING FRAMEWORK V9 - 24H FORECAST (KORRIGIERTE NAMEN)")
print("=" * 70)
...
print(f"Beispiel: Input von {df.index[0]} → Output von {df.index[Config.FORECAST_HORIZON_STEPS]}")
...
# Top 20 Features mit korrigierten Namen
available_features = [f for f in Config.TOP_FEATURES if f in X_all.columns]
missing_features = [f for f in Config.TOP_FEATURES if f not in X_all.columns]
...
print(f"  Verloren durch 24h Shift: {len(df) - len(X)} Zeilen")
...
axes[0].set_title('Test RMSE - 24h Forecast')
...
axes[1].set_ylabel('R²')
axes[1].set_title('Test R² - 24h Forecast')
...
ax.set_title('Overfitting Comparison - 24h Forecast')
...
plot_data_14d = plot_data.loc[start_time:end_time]

ax.plot(plot_data_14d.index, plot_data_14d['true'], label='True', linewidth=1.5)
ax.plot(plot_data_14d.index, plot_data_14d['pred'], label='Predicted (24h ahead)', 
        linewidth=1.5, linestyle='--')
ax.set_title(f'Forecast vs. True (24h ahead) - {best_name}')
ax.set_xlabel('Zeit')
...
ax.set_title(f'Residuals (24h ahead) - {best_name}')
ax.grid(alpha=0.3)
plt.tight_layout()
...
if hasattr(best_model_result['model'], 'feature_importances_'):
    ...
    ax.set_title(f'Feature Importance (24h Forecast) - {best_name}')
    ax.invert_yaxis()
    plt.tight_layout()
...
experiment_log = {
    ...
    'version': 'V9',
    'description': '24h Forecast mit korrigierten Feature-Namen',
    'config': {
        'train_ratio': Config.TRAIN_RATIO,
        'cv_splits': Config.CV_SPLITS,
        'forecast_horizon_steps': Config.FORECAST_HORIZON_STEPS,
        'forecast_horizon_name': Config.FORECAST_HORIZON_NAME,
        'feature_selection': f'Top {len(available_features)} features (korrekte Namen)',
...
print("\n" + "=" * 70)
print("V9 FERTIG - 24H FORECAST MIT KORRIGIERTEN NAMEN")
print("=" * 70)
