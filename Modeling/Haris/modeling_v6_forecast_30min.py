"""
MODELING FRAMEWORK V6 - ECHTES FORECASTING 30MIN VORAUS
========================================================
Alle Features (inkl. Lag_15min und Wetter)
Target: 30min in die Zukunft (shift=-2)
...
class Config:
    # Pfade
    OUTPUT_DIR = r"C:\Users\haris\OneDrive\Anlagen\Desktop\ML nach Model\Energy-Forecasting-Anomaly-Detection-Basel\Modeling\Haris\V6"
    ...
    # Forecast Horizon
    FORECAST_HORIZON_STEPS = 2  # 2 steps = 30min
    FORECAST_HORIZON_NAME = "30min"
...
print("=" * 70)
print(f"MODELING FRAMEWORK V6 - FORECAST {Config.FORECAST_HORIZON_NAME} AHEAD")
print("=" * 70)
...
# ============================================================
# TARGET SHIFT FÜR ECHTES FORECASTING
# ============================================================

print(f"\n[2/5] Target Shift ({Config.FORECAST_HORIZON_NAME} voraus)...")

target_col = "Stromverbrauch"

# WICHTIG: Target in die Zukunft shiften
y = df[target_col].shift(-Config.FORECAST_HORIZON_STEPS).astype(float)

print(f"Target wurde um {Config.FORECAST_HORIZON_STEPS} steps ({Config.FORECAST_HORIZON_NAME}) in die Zukunft verschoben")
print(f"Beispiel:")
print(f"  Input von  {df.index[0]} → Output von {df.index[Config.FORECAST_HORIZON_STEPS]}")
print(f"  Das Modell sagt {Config.FORECAST_HORIZON_NAME} in die Zukunft voraus")
...
# NaNs bereinigen (shift erzeugt NaNs am Ende)
...
print(f"  Verloren durch Shift: {len(df) - len(X)} Zeilen")
...
print(f"\nFeature-Kategorien:")
print(f"  Stromverbrauch-Lags: {len(lag_features)}")
...
# Baseline: Persistence Forecast (letzter bekannter Wert)
if 'Lag_15min' in X_test.columns:
    # Für 30min Forecast: Nehme Wert von vor 15min als Baseline
    ...
    print(f"Baseline (Persistence: Lag_15min) RMSE: {baseline_rmse:.2f}")
...
print("\n" + "=" * 70)
print(f"VERGLEICH - FORECAST {Config.FORECAST_HORIZON_NAME} AHEAD")
print("=" * 70)
...
axes[0].set_title(f'Test RMSE - Forecast {Config.FORECAST_HORIZON_NAME} Ahead')
...
axes[1].set_ylabel('R²')
axes[1].set_title(f'Test R² - Forecast {Config.FORECAST_HORIZON_NAME} Ahead')
...
ax.plot(plot_data_7d.index, plot_data_7d['pred'], label=f'Predicted ({Config.FORECAST_HORIZON_NAME} ahead)', 
        linewidth=1.5, linestyle='--')
ax.set_title(f'Forecast vs. True ({Config.FORECAST_HORIZON_NAME} ahead) - {best_name}')
ax.set_xlabel('Zeit')
...
ax.set_title(f'Residuals ({Config.FORECAST_HORIZON_NAME} ahead) - {best_name}')
ax.grid(alpha=0.3)
plt.tight_layout()
...
experiment_log = {
    ...
    'version': 'V6',
    'description': f'Echtes Forecasting {Config.FORECAST_HORIZON_NAME} voraus mit allen Features',
    'config': {
        'train_ratio': Config.TRAIN_RATIO,
        'cv_splits': Config.CV_SPLITS,
        'forecast_horizon_steps': Config.FORECAST_HORIZON_STEPS,
        'forecast_horizon_name': Config.FORECAST_HORIZON_NAME,
        'feature_selection': 'All features (incl. Lag_15min and Weather)',
    },
    'data': {
    ...
    'baseline': {
        'method': 'Persistence Forecast (Lag_15min)',
...
print("\n" + "=" * 70)
print(f"V6 FERTIG - ECHTES FORECASTING {Config.FORECAST_HORIZON_NAME} VORAUS")
print("=" * 70)
