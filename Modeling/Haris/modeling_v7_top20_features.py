"""
MODELING FRAMEWORK V7 - TOP 20 FEATURES
========================================
30min Forecast mit nur den Top 20 wichtigsten Features
...
class Config:
    # Pfade
    OUTPUT_DIR = r"C:\Users\haris\OneDrive\Anlagen\Desktop\ML nach Model\Energy-Forecasting-Anomaly-Detection-Basel\Modeling\Haris\V7"
    ...
    # Top 20 Features (basierend auf Feature Importance von V6)
    TOP_FEATURES = [
        'Lag_15min',
        'Lag_30min',
        'Lag_1h',
...
print("=" * 70)
print("MODELING FRAMEWORK V7 - TOP 20 FEATURES")
print("=" * 70)
...
print(f"\n[2/5] Target Shift & Feature Selection (Top 20)...")
...
if missing_features:
    print(f"\nFehlende Features (werden übersprungen): {len(missing_features)}")
...
if 'Lag_15min' in X_test.columns:
    ...
    print(f"Baseline (Lag_15min) RMSE: {baseline_rmse:.2f}")
...
print("\n" + "=" * 70)
print("VERGLEICH - TOP 20 FEATURES")
print("=" * 70)
...
axes[0].set_title('Test RMSE - Top 20 Features')
...
axes[1].set_ylabel('R²')
axes[1].set_title('Test R² - Top 20 Features')
...
ax.plot(plot_data_7d.index, plot_data_7d['pred'], label='Predicted (30min ahead)', 
        linewidth=1.5, linestyle='--')
ax.set_title(f'Forecast vs. True (Top 20 Features) - {best_name}')
ax.set_xlabel('Zeit')
...
ax.set_title(f'Residuals (Top 20 Features) - {best_name}')
ax.grid(alpha=0.3)
plt.tight_layout()
...
if hasattr(best_model_result['model'], 'feature_importances_'):
    ...
    ax.set_title(f'Feature Importance (Top 20) - {best_name}')
    ax.invert_yaxis()
    plt.tight_layout()
...
experiment_log = {
    ...
    'version': 'V7',
    'description': '30min Forecast mit Top 20 Features',
    'config': {
        'train_ratio': Config.TRAIN_RATIO,
        'cv_splits': Config.CV_SPLITS,
        'forecast_horizon_steps': Config.FORECAST_HORIZON_STEPS,
        'forecast_horizon_name': Config.FORECAST_HORIZON_NAME,
        'feature_selection': f'Top {len(available_features)} most important features',
...
print("\n" + "=" * 70)
print("V7 FERTIG - TOP 20 FEATURES")
print("=" * 70)
