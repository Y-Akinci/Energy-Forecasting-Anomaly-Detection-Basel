"""
MODELING FRAMEWORK V4 - FEATURE SELECTION
==========================================
Nur Stromverbrauch-Lags, KEINE Wetter-Features
...
class Config:
    # Pfade
    OUTPUT_DIR = r"C:\Users\haris\OneDrive\Anlagen\Desktop\ML nach Model\Energy-Forecasting-Anomaly-Detection-Basel\Modeling\Haris\V4"
...
print("=" * 70)
print("MODELING FRAMEWORK V4 - NO WEATHER FEATURES")
print("=" * 70)
...
# ============================================================
# FEATURE SELECTION - NUR STROMVERBRAUCH LAGS
# ============================================================

print("\n[2/5] Feature Selection (Ohne Wetter)...")
...
# Nur behalten: Lags + Zeit-Features + Kunden
lag_features = [c for c in X_all.columns if 'Lag_' in c]
time_features = ['Monat', 'Wochentag', 'Quartal', 'Woche des Jahres', 'Tag des Jahres']
customer_features = ['Grundversorgte Kunden', 'Freie Kunden']
binary_features = ['IstSonntag', 'IstArbeitstag']

# Kombiniere alle gewünschten Features
keep_features = []
for feat in lag_features + time_features + customer_features + binary_features:
    if feat in X_all.columns:
        keep_features.append(feat)

print(f"\nAusgewählte Features ({len(keep_features)}):")
print(f"  Lag-Features: {len(lag_features)}")
print(f"  Zeit-Features: {len([f for f in time_features if f in X_all.columns])}")
print(f"  Kunden-Features: {len([f for f in customer_features if f in X_all.columns])}")
print(f"  Binary-Features: {len([f for f in binary_features if f in X_all.columns])}")

# Liste aller Features ausgeben
print("\nFeature-Liste:")
for i, feat in enumerate(keep_features, 1):
    print(f"  {i:2d}. {feat}")

X_all = X_all[keep_features]
...
ax.bar(models, test_rmse, alpha=0.8, color='steelblue')
ax.set_xlabel('Model')
ax.set_ylabel('RMSE (kWh)')
ax.set_title('Model Comparison - Test RMSE (No Weather)')
ax.set_xticklabels(models, rotation=45, ha='right')
ax.grid(alpha=0.3, axis='y')
plt.tight_layout()
plt.savefig(os.path.join(Config.OUTPUT_DIR, 'model_comparison_rmse.png'), dpi=Config.PLOT_DPI)
plt.close()
...
ax.set_title(f'Forecast vs. True (No Weather) - {best_name}')
ax.set_xlabel('Zeit')
...
ax.set_title(f'Residuals (No Weather) - {best_name}')
ax.grid(alpha=0.3)
plt.tight_layout()
...
with open(features_list_path, 'w') as f:
    f.write("FEATURES USED IN V4 (NO WEATHER)\n")
    f.write("=" * 50 + "\n\n")
    for i, feat in enumerate(keep_features, 1):
        f.write(f"{i:2d}. {feat}\n")
    f.write(f"\nTotal: {len(keep_features)} features\n")

experiment_log = {
    'timestamp': datetime.now().isoformat(),
    'version': 'V4',
    'description': 'Feature Selection - No Weather Data',
    'config': {
        'train_ratio': Config.TRAIN_RATIO,
        'cv_splits': Config.CV_SPLITS,
        'feature_selection': 'Lags + Time + Customers only (No Weather)',
    },
    'data': {
...
print("\n" + "=" * 70)
print("V4 FERTIG - NUR STROMVERBRAUCH LAGS, KEINE WETTER-DATEN")
print("=" * 70)
